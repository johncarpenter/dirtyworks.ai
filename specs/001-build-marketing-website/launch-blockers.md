...
**Date**: 2026-08-25, revised 2026-09-12 for the workspace-pilot refresh | **Status of the
build**: all merge and deploy gates pass; the site is technically deployable today.
...
|---|---|---|---|---|
| 2 | Approved consent wording for the inquiry form, and an approved privacy notice to link it to | Counsel | `/start` | The form states only what happens to the information (it is used to respond to the inquiry) and references no privacy notice, because none exists. The public `Legal review` stamp was removed from the customer journey by the refresh brief; the dependency is recorded as `INTAKE_CONSENT_WORDING` and still fails `check:launch`. |
| 3 | Privacy notice, website terms, accessibility statement | Counsel | footer | The footer carries no legal column at all: no inert labels, no links to empty pages (FR-008). |
...
| 7 | Confirmed supported product shortlist and any authorized-reseller statements | Sponsor | `/catalogue` | Every product name renders as a candidate example under a `VERIFY AT QUOTE` stamp, with no price and no purchase route. The page now states that products are separate vendor products, and that which of them connect inside a workspace pilot is confirmed at scoping. |
| 9 | Workspace pilot inputs: a real demonstration asset with sample data (recording, poster, captions); pilot support terms and any publicly stated limits; the verified account-control and handover model for partner-provisioned workspaces; any intended Cloudflare partner designation and its use rights | Founder | `/`, `/workspace` | Both pages ship the brief's conservative proposed copy. No duration, cohort size, fee, usage allowance, partner badge, or product screenshot is stated; the hero visual is a text example stamped `Illustrative workflow`. Recorded as `WORKSPACE_PILOT_INPUTS` in `src/copy/placeholders.ts`. **Do not fill these gaps with invented facts.** |
...
| Item | Evidence |
|---|---|
| At least one visibly illustrative operating example | 9 registered claim artefacts, each verified by the release gate to render its stamp |
| No fabricated logos, testimonials, certifications, results or vendor relationships | Release gate rules 4 and 5; `tests/e2e/home.spec.ts` asserts zero images, zero price patterns, and no partner-badge wording |
| Three complete Notes articles *if* Notes launches | `/notes` ships as an honest empty state — the public backlog of unwritten titles was removed by the refresh — reachable from the footer only, with no dates or reading times |
...
| Mobile QA | `tests/e2e/responsive.spec.ts` — 10 routes × 6 widths from 320px |
